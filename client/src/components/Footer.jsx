...
const socialIcons = [
    { icon: FacebookIcon, link: "https://www.facebook.com/profile.php?id=100073180518757&rdid=LOIp6Q7kwHs3exfh&share_url=https%3A%2F%2Fwww.facebook.com%2Fshare%2F1HpzsnpmXK%2F#)" },
    ...
    return (
        <footer>
            <div className="flex flex-col md:flex-row items-start justify-center px-6 md:px-16 lg:px-32 gap-10 py-14 border-b border-gray-500/30 text-gray-500">
                <div className="w-4/5">
                    <h1 className="text-3xl font-bold">Stay Villa</h1>
                    <p className="mt-6 text-sm">
                        Discover the world's most extraordinary places to stay, from boutique hotels to luxury villas and private islands. Experience unparalleled comfort and service.
                    </p>
                                ...
                                <ul className="space-y-2.5">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2">
                                            {link.icon && <link.icon />}
                                            <a href={link.path} className="hover:underline transition">{link.text}</a>
                                        </li>
                                    ))}
                                </ul>
                            </div>
            ...
            <p className="py-4 text-center text-xs md:text-sm">
                Copyright {new Date().getFullYear()} © StayVilla. All Right Reserved.
            </p>
        </footer>
    )
}
